...
# ------------------------------------------------------------------------
# reconciliate catalogue entries that refer to the same documents.
# forked+adapted from the reconciliator in the katabase app
# (https://github.com/katabase/Application/blob/main/APP/reconciliator.py)
# ------------------------------------------------------------------------
...
def double_loop(input_dict):
    ...
    :return: 3 lists
    """
    output_dict1 = {}  # dict mapping 'id1-id2' to {'score': N, 'author_distance': N} to store comparisons between items
    processed = []  # list to store all ids that we have processed reconciliated, to avoid duplicate processing
    # sensibility scores. an item must have scores > 0.6 for the desc and > 0.4 for the author to be reconciliated
    sensibility_item = 0.6  # score above which 2 items are considered reconciliated
    sensibility_auth = 0.4  # score above which 2 authors are reconciliated
    ...
    for i in tqdm.tqdm(range(len(items))):
        # ========= prepare the data ========= #
        id_a, desc_a = items[i]
        processed.append(id_a)  # the current item is added to the list of processed items
        desc_a["cat_id"] = id_a
        entry_id_a = id_a
        desc_a["cat_entry"] = entry_id_a

        # ========= check for all matching items in input_dict ========= #
        # try to reconciliate i with all other items in input_dict
        ...
        for j in range(i + 1, len(items)):
            ...
            if entry_id_a == entry_id_b or entry_id_b in processed:
                print("y")
                continue
            ...
            # considering that mss can only be reconciliated with itself, input_dict[id_b]
            # won't be processed again if the scores indicate a reconciliation
            if score_entry["score"] > sensibility_item and score_entry["author_distance"] >= sensibility_auth:
                processed.append(id_b)
    ...
    for key in output_dict1:
        ...
        final_list.append((
            output_dict1[key]["score"],
            [first_entry, second_entry],
            output_dict1[key]["author_distance"],
            {first_entry: input_dict[first_entry]},
            {second_entry: input_dict[second_entry]}
        ))
    ...
    filtered_list_with_score = [[item[1], item[0]] for item in final_list
                                if item[0] > sensibility_item and item[2] >= sensibility_auth]
...
def reconciliator():
    """
    reconciliate all matching entries in export_item.json
    :return:
    """
    with open("../in/export_item.json", mode="r") as fh:
        data = json.load(fh)
...
"""
    ...
    final_results["score"] = results_lists[0]  # list of matching entries'id with scores
    final_results["groups"] = results_lists[1]  # list of matching entries'ids with the json data
    final_results["recon_desc"] = results_lists[2]  # list of reconciliated descs
